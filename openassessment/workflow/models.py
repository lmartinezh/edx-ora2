...
class AssessmentWorkflow(TimeStampedModel, StatusModel):
    ...
    def get_score(self, assessment_requirements, step_for_name):
        # We're trying to give a score. We currently have a
    ...
    def cancel(self, assessment_requirements):
        ...
        Set the the points earned to 0 and workflow status to cancelled.
        ...
        # Finally save our changes if the status is not cancelled.
        if self.status != self.STATUS.cancelled:
            self.status = self.STATUS.cancelled
            self.save()
            logger.info((
                        u"Workflow for submission UUID {uuid} has updated status to {status}"
                        ).format(uuid=self.submission_uuid, status=self.STATUS.cancelled))
